...
public class FrameProcessInt
{
     public boolean startProcess(String videopath)
  {
    boolean flag=false;
     try
         
     {
         //System.out.println("Video path in FrameProcessInit "+videopath);
    ...
    if (videoCoder.open() < 0)
      throw new RuntimeException(
        "could not open video decoder for container: " + videopath);
    ...
    if (videoCoder.getPixelType() != IPixelFormat.Type.BGR24)
    {
      ...
      if (resampler == null)
        throw new RuntimeException(
          "could not create color space resampler for: " + videopath);
    }
    ...
    while(container.readNextPacket(packet) >= 0)
    {
      ...
      if (packet.getStreamIndex() == videoStreamId)
      {
        ...
        while(offset < packet.getSize())
        {
          ...
          if (bytesDecoded < 0)
            throw new RuntimeException("got error decoding video in: " + videopath);
          offset += bytesDecoded;
          ...
          if (picture.isComplete())
          {
            ...
            if (resampler != null)
            {
              ...
              if (resampler.resample(newPic, picture) < 0)
                throw new RuntimeException(
                  "could not resample video from: " + videopath);
            }

            if (newPic.getPixelType() != IPixelFormat.Type.BGR24)
              throw new RuntimeException(
                "could not decode video as BGR 24 bit data in: " + videopath);
            ...
            // process the video frame
             ...
             // System.out.println("Reached till extracting the frames");

            new FrameSupporterInt().processFrame(newPic, javaImage);
          }
        }
      }
      else
